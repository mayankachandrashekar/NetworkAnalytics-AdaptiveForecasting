...
/**
  ...
  */
object AdaptiveDynamicForecasting {
  def main(args: Array[String]): Unit = {
    ...
    val result=groupedDataset.map(f=>{
      ...
      ff.foreach(f=>{
        if(j<5&&j>0){
          ...
          if (j == 5) {
            val error = MAD(original, forecast)
    ...
    val out=new PrintStream("G:\\Dropbox\\2016 Fall\\Network Analytics\\Network Analytics-Assignment\\Assignment\\Assignment 3\\AdaptiveResult3.csv")
